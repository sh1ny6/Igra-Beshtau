...
window.addEventListener('load', () => {
  ...
  scene('start', () => {
    ...
    for (let i = 0; i < maxClouds; i++) {
      clouds.push(
        add([
          'cloud',
          sprite('cloud', {
            width: 300 * (randi(50, 150) / 100),
            flipX: rand(10) > 5 ? true : false,
          }),
          anchor('center'),
          z(-1),
          fixed(),
          ...
          pos(
            gameWidth * (1 + randi(7, 50) / 100),
            gameHeight * 0.1 + i * randi(30, 50)
          ),
          {
            speed: randi(2, 8) * 10,
            isActive: true,
    ...
    let logoHeight = 260;
    add([
      sprite('screenWelcomeLogo', {
        width: logoWidth,
        height: logoHeight,
      }),
      pos(gameWidth / 2 - (logoWidth * scaleCoef) / 2, 246 * scaleCoef),
      scale(scaleCoef),
    ]);
    ...
    add([
      sprite('screenWelcomeCaption', {
        width: captionWidth,
        height: captionHeight,
      }),
      pos(gameWidth / 2 - (captionWidth * scaleCoef) / 2, 530 * scaleCoef),
      scale(scaleCoef),
    ]);
    ...
    add([
      'startButton',
      sprite('screenWelcomeBtn', {
        width: btnWidth,
        height: btnHeight,
      }),
      pos(gameWidth / 2 - (btnWidth * scaleCoef) / 2, 700 * scaleCoef),
      scale(scaleCoef),
      area(),
    ]);
    ...
    add([
      'btnChange',
      sprite('designChangeBtn', {
        width: btnChangeWidth,
        height: btnChangeHeight,
      }),
      pos(
        gameWidth - (btnChangeWidth * scaleCoef) / 2 - 20,
        gameHeight / 7 - (btnChangeHeight * scaleCoef) / 2
      ),
      scale(scaleCoef),
      anchor('center'),
      z(1500),
  ...
  scene('tutorial', () => {
    ...
    add([
      'startButton',
      sprite('screenTutorialBtnContinue', { height: 90 * scaleCoef }),
      pos(gameWidth / 2, gameHeight * 0.9),
      anchor('center'),
      area(),
    ]);
    onClick('startButton', () => {
  ...
  scene('select', () => {
    ...
    let logoHeight = 144;
    add([
      sprite('screenSelectLogo', {
        width: logoWidth,
        height: logoHeight,
      }),
      pos(108 * scaleCoef, 63 * scaleCoef),
      scale(scaleCoef),
    ]);
    ...
    let charName = add([
      ...
      pos(
        gameWidth / 2 - textWidth / 2,
        posY + boxHeight / 2 - textSize / 2 + outlineWidth / 2
      ),
      fixed(),
      z(1500),
      color(Color.fromHex('#009657')),
    ]);
    ...
    let charText = add([
      ...
      pos(gameWidth / 2 - charTextWidth / 2, gameHeight * 0.7),
      fixed(),
      z(1500),
      color(Color.WHITE),
    ]);
    ...
    characters.push({
      ...
      description:
        'Суров, бородат и могуч. Способен с легкостью собрать любой монитор и компьютер, а еще покорить любую вершину.',
    });
    ...
    characters.push({
      ...
      description:
        'Аналитический склад ума и изобретательский опыт. Может разработать и реализовывать план покорения любых вершин.',
    });
    ...
    characters.push({
      ...
      description:
        'Красива, умна и изобретательна. Легко очаровывает любые горы и покоряет вершины.',
    });
    ...
    add([
      'btnPrev',
      sprite('screenSelectBtnPrevNext', {
        width: btnPrevWidth,
        height: btnPrevHeight,
      }),
      pos(gameWidth / 4, gameHeight / 2),
      scale(scaleCoef),
      anchor('center'),
      fixed(),
      z(1500),
    ...
    add([
      ...
      pos((gameWidth / 4) * 3, gameHeight / 2),
      scale(scaleCoef),
      anchor('center'),
      fixed(),
      z(1500),
    ...
    add([
      'btnStart',
      sprite('screenSelectBtnStart', {
        width: btnSelectWidth,
        height: btnSelectHeight,
      }),
      pos(gameWidth / 2, gameHeight * 0.95 - (btnSelectHeight * scaleCoef) / 2),
      scale(scaleCoef),
      anchor('center'),
      z(1500),
      fixed(),
  ...
  scene('game', (selectedCharacter) => {
    ...
    const map = addLevel(
      ...
      ],
      {
        ...
        tiles: {
          /*--- GROUND ---*/
          0: () => [
            rect(tileSize / tileScale, tileSize / tileScale),
            opacity(0),
            area(),
            scale(tileScale),
            body({ isStatic: true }),
          ...
          1: () => [
            ...
            'deep',
            'hollow',
            sprite('ground-deep-1'),
            scale(tileScale),
          ],
          2: () => [
            'tile-ground',
            sprite('ground-top-left'),
            area(),
            scale(tileScale),
            body({ isStatic: true }),
          ],
          3: () => [
            'tile-ground',
            sprite('ground-top-center'),
            area({
              shape: new Rect(
                vec2(0),
                Math.round(tileSize / tileScale),
                Math.round(tileSize / tileScale)
              ),
            }),
            scale(tileScale),
          ...
          A: () => [
            'bonus-shield',
            sprite('bonus-shield', { width: 22, height: 22 }),
            area(),
            pos(tileSize / 5, tileSize / 4 - 1),
          ],
          P: () => [
            'bonus-health',
            sprite('bonus-maxHealth', { width: 22, height: 22 }),
            area(),
            pos(tileSize / 5, tileSize / 4 - 1),
          ],
          R: () => [
            'bonus-double',
            sprite('bonus-double', { width: 22, height: 22 }),
            area(),
            pos(tileSize / 5, tileSize / 4 - 1),
          ],
          /*--- DANGER ---*/
          B: () => [
            'danger-top',
            sprite('spikes_bottom', { width: tileSize }),
            area({
              shape: new Rect(vec2(0), tileSize, Math.round(tileSize * 0.3)),
            }),
            anchor('botleft'),
            pos(vec2(0, tileSize)),
            body({ isStatic: true }),
          ...
          C: () => [
            'danger',
            sprite('wires', { width: tileSize, flipX: randi() }),
            area({
              shape: new Rect(vec2(0), tileSize * 0.5, 20 * scaleCoef),
              offset: vec2(0, -1),
            }),
            anchor('center'),
            pos(vec2(tileSize / 2, tileSize + 2)),
            z(1500),
          ],
          D: () => [
            'danger-move',
            sprite('spikes_top', { width: tileSize }),
            anchor('top'),
            pos(vec2(tileSize / 2, -1 * tileSize)),
            area(),
            body({ isStatic: true }),
            { speed: -20, reverse: false, moveTimer: new Timer(3) },
          ],
          E: () => [
            'danger-arc',
            anchor('center'),
            pos(vec2(tileSize / 2, 0)),
            {
              chargeTimer: new Timer(4, () => {
                return false;
              }),
              sfx: play('electric', { volume: 0.4, paused: true }),
            },
          ],
          F: () => [
            'danger-transformer',
            sprite('transformer', { width: 22 }),
            anchor('center'),
            pos(vec2(tileSize / 2, tileSize * 0.5)),
            {
              chargeTimer: new Timer(4, () => {
                return false;
              }),
              sfx: play('transformer', { volume: 0.4, paused: true }),
            },
          ],
          /*--- COLLECTABLES ---*/
          '*': () => [
          ...
          W: () => [
            'win',
            sprite('win-flag', { height: 50 * scaleCoef }),
            area({
              shape: new Rect(
                vec2(0),
                Math.round(30 * scaleCoef),
                Math.round(50 * scaleCoef)
              ),
              offset: vec2(2, -2),
            }),
            anchor('botleft'),
            pos(vec2(0, tileSize + 2)),
            z(1300),
          ],
        },
      }
    ...
    map.children.forEach((tile) => {
      if (tile.is('interactable')) {
        ...
        tile.add([
          'text',
          sprite('repair-sign-2', { height: 10 }),
          anchor('top'),
          z(2000),
          pos(30, -50),
        ]);
        ...
        tile.get('progress-bar').forEach((e) => {
          //outer bar outline
          e.add([
            rect(e.boxWidth, e.boxHeight, { radius: e.boxRadius, fill: false }),
            pos(0, 0),
            outline(0.8, Color.fromHex('#5D5D5D')),
          ]);
          //inner bar outline
          e.add([
            rect(e.boxWidth - 2, e.boxHeight - 2, {
              radius: e.boxRadius * 0.5,
              fill: false,
            }),
            pos(1, 1),
            outline(0.6, Color.fromHex('#5D5D5D')),
            color(Color.fromHex('#5D5D5D')),
          ]);
          //inner bar color - ADJUSTABLE WIDTH HERE
          e.add([
            'bar',
            rect(0, e.boxHeight - 2, {
              radius: e.boxRadius * 0.5,
              fill: false,
            }),
            pos(1, 1),
            color(Color.fromHex('#ff7d04')),
          ]);
          //text
          e.add([
            sprite('repair-text', { width: e.boxWidth - 4 }),
            pos(2, -1 * (e.boxHeight + 1)),
          ]);
        });
      } else if (tile.is('danger')) {
        tile.add([
          'effect',
          sprite('effect-scull', { width: 7 }),
          anchor('center'),
          pos(vec2(0, -35)),
        ]);
        ...
        tile.chargeTimer.action = () => {
          tile.get('arc-effect')[0].hidden = false;
          wait(1, () => {
            tile.get('arc-effect')[0].hidden = true;
            tile.chargeTimer.reset(4);
          });
        };
        tile.add([
        ...
        tile.moveTimer.action = () => {
          wait(0.5, () => {
            tile.reverse = !tile.reverse;
            tile.moveTimer.reset(2);
          });
        };
        tile.add([
          'effect',
        ...
        tile.chargeTimer.action = () => {
          let explode = tile.get('transformer-explode')[0];
          explode.hidden = false;
          explode.play('explode-anim', { speed: 8 });
          wait(0.5, () => {
            tile.get('transformer-explode')[0].hidden = true;
            tile.chargeTimer.reset(4);
          });
        };
        tile.add([
        ...
        tile.add([
          'text',
          text('Чтобы покорить вершину, собери все части Российского ПК', {
            font: 'Inter',
            size: 6,
            width: 120,
            align: 'center',
          }),
          /*color(Color.fromHex('#ff7d04'))*/ color(Color.WHITE),
          anchor('bot'),
          pos(30 * scaleCoef, -45),
          z(2900),
        ]).hidden = true;
    ...
    const player = add([
      ...
      playerSpawnPos,
      {
        ...
        afterHitDelay: 3,
        blinkFrequency: 1,
        doubleTimer: new Timer(0, () => {
          return false;
        }),
        shieldTimer: new Timer(0, () => {
          return false;
        }),
        isDouble: () => {
          return player.doubleTimer.time > 0;
        },
        isShielded: () => {
          return player.shieldTimer.time > 0;
        },
        isBlocked: false,
        collectedParts: {
          comp: false,
          ...
          isComplete: () => {
            return (
              player.collectedParts.comp &&
              player.collectedParts.mouse &&
              player.collectedParts.keyboard &&
              player.collectedParts.monitor
            );
          },
        },
        canMove: {
    ...
    if (isMobile() == true) {
      ...
      onMouseDown('left', () => {
        if (btnLeft == true) {
          if (!player.isBlocked) {
            if (
              player.curAnim() !== selectedCharacter.animations.run &&
              player.isGrounded()
            ) {
        ...
        interactable.forEach((obj) => {
          if (player.isColliding(obj)) {
            if (obj.isInteractable && !obj.isRepaired && !player.isBlocked) {
              ...
              obj.get('repair-cloud').forEach((e) => {
                e.hidden = false;
                e.play('repair-anim');
                console.log(1);
              });
              obj.get('progress-bar').forEach((e) => {
                e.hidden = false;
              });
              obj.get('text').forEach((e) => {
                e.destroy();
              });
              obj.repairTimer.action = () => {
                ...
                obj.use(
                  pos(
                    obj.parent.tileWidth() * obj.tilePos.x,
                    obj.parent.tileHeight() * (obj.tilePos.y + 1) + 2
                  )
                );
                obj.use(z(-1));
                ...
                player.isBlocked = false;
                let score = player.isDouble()
                  ? SETTINGS.score.interactable * 2
                  : SETTINGS.score.interactable;
                player.score += score;
                checkScore(player);
                ...
                for (let i = 0; i < maxEffects; i++) {
                  if (collectableEffects[i].free)
                    effect = collectableEffects[i];
                }
                if (!effect) return;
                effect.free = false;
                effect.hidden = false;
    ...
    onDraw(() => {
      ...
      for (let i = 1; i <= player.maxHealth; i++) {
        if (i <= player.hp()) {
          drawSprite({
            ...
            fixed: true,
            pos: vec2(
              gameWidth * 0.05 + (heartWidth + heartGap) * scaleCoef * i,
              interfaceTextPosY
            ),
          });
        } else {
      ...
      let scoreBox = {
        width: Math.max(
          scoreTextFormat.width * 1.3 * scaleCoef,
          gameWidth * 0.11
        ),
      ...
      drawText({
        ...
        pos: vec2(
          scoreBox.posX + 1,
          scoreBox.posY + interfaceTextSize * 0.1 + 1
        ),
      ...
      drawSprite({
        ...
        pos: vec2(
          gameWidth * 0.83,
          interfaceTextPosY - interfaceTextSize * 0.05
        ),
      });
      ...
      for (let i = 0; i < activeBonuses.length; i++) {
        ...
        bonusBoundingWidth +=
          bonusBox[i].width + bonusBox[i].height * 0.15 + (i ? bonusBoxGap : 0); // 0.15 for sprite being 1.3 the h and w of box
      }
      ...
      for (let i = 0; i < activeBonuses.length; i++) {
        bonusBox[i].posX =
          gameWidth * 0.5 -
          bonusBoundingWidth * 0.5 +
          bonusBox[i].height * 0.15 +
          i * (bonusBox[i].width + bonusBoxGap); // calculated
        drawMasked(
          () => {
            drawRect({
        ...
        drawText({
          ...
          pos: vec2(
            bonusBox[i].posX + 1,
            interfaceTextPosY + interfaceTextSize * 0.1 + 1
          ),
          color: Color.fromHex('#343434'),
          fixed: true,
        });
        drawText({
          ...
          pos: vec2(
            bonusBox[i].posX,
            interfaceTextPosY + interfaceTextSize * 0.1
          ),
          color: rgb(255, 255, 255),
          fixed: true,
        });
        drawSprite({
          ...
          fixed: true,
          pos: vec2(
            bonusBox[i].posX - bonusBox[i].height * 0.15,
            interfaceTextPosY - bonusBox[i].height * 0.15
          ),
        });
      }
      ...
      if (player.collectedParts.isComplete()) {
        ...
        if (player.collectedParts.mouse) {
          drawSprite({
            ...
            fixed: true,
            pos: vec2(
              contourPosition.x + 164 * scaleCoef,
              contourPosition.y + 118 * scaleCoef
            ),
          });
        }
        if (player.collectedParts.comp) {
          drawSprite({
            ...
            fixed: true,
            pos: vec2(
              contourPosition.x + 122 * scaleCoef,
              contourPosition.y + 18 * scaleCoef
            ),
          });
        }
        if (player.collectedParts.monitor) {
          drawSprite({
            ...
            fixed: true,
            pos: vec2(
              contourPosition.x + -10 * scaleCoef,
              contourPosition.y + 8 * scaleCoef
            ),
          });
        }
        if (player.collectedParts.keyboard) {
          drawSprite({
            ...
            fixed: true,
            pos: vec2(
              contourPosition.x + 20 * scaleCoef,
              contourPosition.y + 89 * scaleCoef
            ),
          });
        }
      }
    ...
    loop(1, () => {
      ...
      map.get('interactable').forEach((e) => {
        if (e.isActive) {
          ...
          e.get('progress-bar').forEach((r) => {
            r.get('bar').forEach((b) => {
              b.width = (r.boxWidth - 2) * ((5 - e.repairTimer.time) / 5);
            });
          });
        }
      });
      ...
      map.get('danger-arc').forEach((tile) => {
        tile.chargeTimer.tick(1);
      });
      map.get('danger-transformer').forEach((tile) => {
        tile.chargeTimer.tick(1);
      });
      map.get('danger-move').forEach((tile) => {
        tile.moveTimer.tick(1);
      });
    ...
    player.onCollide('danger-top', (obj, e) => {
      if (e.isBottom())
        playerDamage(player, obj, maxEffects, collectableEffects);
    });
    player.onCollide('danger-move', (obj, e) => {
      if (e.isTop()) playerDamage(player, obj, maxEffects, collectableEffects);
    });

    player.onCollide('danger', (obj, e) =>
      playerDamage(player, obj, maxEffects, collectableEffects)
    );
    player.onCollide('arc-effect', (obj, e) =>
      playerDamage(player, obj, maxEffects, collectableEffects)
    );
    player.onCollideUpdate('arc-effect', (obj, e) =>
      playerDamage(player, obj, maxEffects, collectableEffects)
    );
    player.onCollideUpdate('transformer-explode', (obj, e) =>
      playerDamage(player, obj, maxEffects, collectableEffects)
    );
    ...
    onUpdate(() => {
      ...
      player.heightDelta = player.previousHeight
      ...
      for (let i = 0; i < maxEffects; i++) {
        if (!collectableEffects[i].free)
          collectableEffects[i].move(0, -1 * collectableEffects[i].speed);
      }
      map.get('danger-move').forEach((tile) => {
  ...
  scene('restart', (score) => {
    ...
    add([
      'btnReset',
      sprite('screen-lose-btn', { width: Math.min(150, gameWidth * 0.2) }),
      pos(gameWidth / 2, gameHeight * 0.9),
      scale(scaleCoef),
      anchor('center'),
      z(1500),
  ...
  scene('win', (score) => {
    ...
    add([
      'btnReset',
      sprite('screen-win-btn', { width: Math.min(300, gameWidth * 0.6) }),
      pos(gameWidth / 2, gameHeight * 0.9),
      scale(scaleCoef),
      anchor('center'),
      z(1500),
  ...
  function checkScore(player) {
    if (player.score >= SETTINGS.score.parts.monitor) {
      ...
      if (!player.collectedParts.keyboard)
        player.collectedParts.keyboard = true;
      ...
      player.score >= SETTINGS.score.parts.keyboard &&
      !player.collectedParts.keyboard
    )
      player.collectedParts.keyboard = true;
  }
...
function initAssets() {
  ...
  loadSprite(
    'background-0',
    `assets/img/game_visual_${gameVisual}/background/background-0.png`
  );
  loadSprite(
    'background-1',
    'assets/img/game_visual_1/background/background-1.png'
  );
  loadSprite(
    'background-2',
    'assets/img/game_visual_1/background/background-2.png'
  ); // передний план

  loadSprite(
    'cloud',
    `assets/img/game_visual_${gameVisual}/environment/cloud.png`
  );

  //welcome screen sprites
  loadSprite(
    'screenWelcomeLogo',
    `assets/img/game_visual_${gameVisual}/screens/welcome/logo.png`
  );
  loadSprite(
    'screenWelcomeCaption',
    `assets/img/game_visual_${gameVisual}/screens/welcome/caption.png`
  );
  loadSprite(
    'screenWelcomeBtn',
    `assets/img/game_visual_${gameVisual}/screens/welcome/btn_v2.png`
  );

  //tutorial
  loadSprite(
    'screenTutorialContent',
    `assets/img/game_visual_${gameVisual}/screens/tutorial/tutorial_complete.png`
  );
  loadSprite(
    'screenTutorialBtnContinue',
    `assets/img/game_visual_${gameVisual}/screens/tutorial/btn_continue.png`
  );
  loadSprite(
    'screenTutorialBtnClose',
    `assets/img/game_visual_${gameVisual}/screens/tutorial/btn_close.png`
  );
  loadSprite(
    'screenTutorialBtnOpen',
    `assets/img/game_visual_${gameVisual}/screens/tutorial/btn_open.png`
  );

  //select screen sprites
  loadSprite(
    'screenSelectLogo',
    `assets/img/game_visual_${gameVisual}/screens/select/logo_small.png`
  );
  loadSprite(
    'screenSelectBtnPrevNext',
    `assets/img/game_visual_${gameVisual}/screens/select/btn_prev_next.png`
  );
  loadSprite(
    'screenSelectBtnStart',
    `assets/img/game_visual_${gameVisual}/screens/select/btn_select.png`
  );
  loadSprite(
    'designChangeBtn',
    `assets/img/game_visual_${gameVisual}/screens/tutorial/btn.png`
  );
  loadSprite(
    'btnUp',
    `assets/img/game_visual_${gameVisual}/buttons/btn_up.png`
  );
  loadSprite(
    'btnLeft',
    `assets/img/game_visual_${gameVisual}/buttons/btn_left.png`
  );
  loadSprite(
    'btnRight',
    `assets/img/game_visual_${gameVisual}/buttons/btn_right.png`
  );
  loadSprite(
    'btnEnter',
    `assets/img/game_visual_${gameVisual}/buttons/btn_enter.png`
  );

  // ground tiles
  loadSpriteAtlas(
    `assets/img/game_visual_${gameVisual}/environment/tileset.png`,
    {
  ...
  loadSprite(
    'platform',
    `assets/img/game_visual_${gameVisual}/environment/platform.png`
  );
  loadSprite(
    'spikes_bottom',
    `assets/img/game_visual_${gameVisual}/environment/spikes_bottom.png`
  );
  loadSprite(
    'spikes_top',
    `assets/img/game_visual_${gameVisual}/environment/spikes_top.png`
  );
  loadSprite(
    'wires',
    `assets/img/game_visual_${gameVisual}/environment/wires.png`
  );
  loadSprite(
    'effect-scull',
    `assets/img/game_visual_${gameVisual}/effects/danger_scull.png`
  );
  loadSprite(
    'arc-ball',
    `assets/img/game_visual_${gameVisual}/environment/arc_ball.png`
  );
  loadSprite(
    'arc-effect',
    `assets/img/game_visual_${gameVisual}/environment/arc_lightning.png`
  );
  loadSprite(
    'transformer',
    `assets/img/game_visual_${gameVisual}/environment/transformer.png`
  );
  loadSprite(
    'transformer-explode',
    `assets/img/game_visual_${gameVisual}/environment/explode.png`,
    {
      sliceX: 4,
      sliceY: 1,
  ...
  loadSprite(
    'char1_idle',
    `assets/img/game_visual_${gameVisual}/character/char_1_idle.png`
  );
  loadSprite(
    'char1_big',
    `assets/img/game_visual_${gameVisual}/character/char_1_big.png`
  );
  loadSprite(
    'char1_jump',
    `assets/img/game_visual_${gameVisual}/character/char_1_jump.png`
  );
  loadSprite(
    'char1_walk',
    `assets/img/game_visual_${gameVisual}/character/char_1_walk.png`,
    {
      sliceX: 4,
      sliceY: 1,
  ...
  loadSprite(
    'char2_idle',
    `assets/img/game_visual_${gameVisual}/character/char_2_idle.png`
  );
  loadSprite(
    'char2_big',
    `assets/img/game_visual_${gameVisual}/character/char_2_big.png`
  );
  loadSprite(
    'char2_jump',
    `assets/img/game_visual_${gameVisual}/character/char_2_jump.png`
  );
  loadSprite(
    'char2_walk',
    `assets/img/game_visual_${gameVisual}/character/char_2_walk.png`,
    {
      sliceX: 4,
      sliceY: 1,
  ...
  loadSprite(
    'char3_idle',
    `assets/img/game_visual_${gameVisual}/character/char_3_idle.png`
  );
  loadSprite(
    'char3_big',
    `assets/img/game_visual_${gameVisual}/character/char_3_big.png`
  );
  loadSprite(
    'char3_jump',
    `assets/img/game_visual_${gameVisual}/character/char_3_jump.png`
  );
  loadSprite(
    'char3_walk',
    `assets/img/game_visual_${gameVisual}/character/char_3_walk.png`,
    {
      sliceX: 4,
      sliceY: 1,
  ...
  loadSprite(
    'heart-outline',
    `assets/img/game_visual_${gameVisual}/objects/heart_outline.png`
  );
  loadSprite(
    'heart-empty',
    `assets/img/game_visual_${gameVisual}/objects/heart_empty.png`
  );
  loadSprite(
    'bonus-maxHealth',
    `assets/img/game_visual_${gameVisual}/objects/bonus_max_health.png`
  );
  loadSprite(
    'bonus-double',
    `assets/img/game_visual_${gameVisual}/objects/bonus_double.png`
  );
  loadSprite(
    'bonus-shield',
    `assets/img/game_visual_${gameVisual}/objects/bonus_shield.png`
  );

  loadSprite(
    'computer-contour',
    `assets/img/game_visual_${gameVisual}/objects/computer_contour.png`
  );
  loadSprite(
    'computer-complete',
    `assets/img/game_visual_${gameVisual}/objects/computer_contour_complete.png`
  );
  loadSprite(
    'computer-keyboard',
    `assets/img/game_visual_${gameVisual}/objects/computer_keyboard.png`
  );
  loadSprite(
    'computer-keyboard-collectible',
    `assets/img/game_visual_${gameVisual}/objects/computer_keyboard_collectible.png`
  );
  loadSprite(
    'computer-monitor',
    `assets/img/game_visual_${gameVisual}/objects/computer_monitor.png`
  );
  loadSprite(
    'computer-monitor-collectible',
    `assets/img/game_visual_${gameVisual}/objects/computer_monitor_collectible.png`
  );
  loadSprite(
    'computer-mouse',
    `assets/img/game_visual_${gameVisual}/objects/computer_mouse.png`
  );
  loadSprite(
    'computer-mouse-collectible',
    `assets/img/game_visual_${gameVisual}/objects/computer_mouse_collectible.png`
  );
  loadSprite(
    'computer-case',
    `assets/img/game_visual_${gameVisual}/objects/computer_case.png`
  );

  loadSprite(
    'workstation-1',
    `assets/img/game_visual_${gameVisual}/objects/workstation_1.png`
  );
  loadSprite(
    'workstation-1-broken',
    `assets/img/game_visual_${gameVisual}/objects/workstation_1_broken.png`
  );
  loadSprite(
    'workstation-2',
    `assets/img/game_visual_${gameVisual}/objects/workstation_2.png`
  );
  loadSprite(
    'workstation-2-broken',
    `assets/img/game_visual_${gameVisual}/objects/workstation_2_broken.png`
  );
  loadSprite(
    'workstation-3',
    `assets/img/game_visual_${gameVisual}/objects/workstation_3.png`
  );
  loadSprite(
    'workstation-3-broken',
    `assets/img/game_visual_${gameVisual}/objects/workstation_3_broken.png`
  );
  loadSprite(
    'repair-sign',
    `assets/img/game_visual_${gameVisual}/effects/repair_sign.png`
  );
  loadSprite(
    'repair-sign-2',
    `assets/img/game_visual_${gameVisual}/effects/repair_sign_2.png`
  );
  loadSprite(
    'repair-text',
    `assets/img/game_visual_${gameVisual}/effects/repair_text.png`
  );
  loadSprite(
    'repair-cloud',
    `assets/img/game_visual_${gameVisual}/effects/repair_cloud_r.png`,
    {
      sliceX: 4,
      sliceY: 1,
  ...
  loadSprite(
    'win-flag',
    `assets/img/game_visual_${gameVisual}/objects/win_flag.png`
  );
  loadSprite(
    'screen-win-flag',
    `assets/img/game_visual_${gameVisual}/screens/endgame/flag_win.png`
  );
  loadSprite(
    'screen-win-btn',
    `assets/img/game_visual_${gameVisual}/screens/endgame/btn_win.png`
  );
  loadSprite(
    'screen-lose-heart',
    `assets/img/game_visual_${gameVisual}/screens/endgame/heartbreak_lose.png`
  );
  loadSprite(
    'screen-lose-btn',
    `assets/img/game_visual_${gameVisual}/screens/endgame/btn_lose.png`
  );
...
function updateTiles(map, player, mapScale = 3) {
  map.children.forEach((tile) => {
    ...
    let dist =
      (player.pos.x - tileX) * (player.pos.x - tileX) +
      (player.pos.y - tileY) * (player.pos.y - tileY);
    if (dist > threshold) {
      tile.hidden = true;
      tile.paused = true;
    } else {
